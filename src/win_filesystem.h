...
{
	bool CreateFilePathList(const wchar_t* folderPath, const wchar_t* fileSpec, std::vector<std::wstring>& paths, bool toAddParent = true);
	...
	bool CreateDirectoryStatic(std::wstring_view directoryName, wchar_t* dst, size_t& nWritten, size_t dstSize, std::wstring_view basePath = {});

	std::string LoadFileAsString(const wchar_t* filePath);
	bool SaveStringToFile(const wchar_t* filePath, const void* pData, unsigned long dataLength, bool toOverWrite = true);
